...
import { AUTH } from '../../constants/urls.js'
...
export default function Login() {
  ...
  const validationSchema = useMemo(
    () =>
      object({
        email: string().email(t('login.err_email')).required(t('login.err_email_required')),
        password: string().min(9, t('login.err_password')).required(t('login.err_password_required')),
      }),
    [t]
  )
    ...
    <>
      <div className="h-screen w-screen bg-gradient-to-tl from-primary/70 to-white flex flex-col justify-center items-center">
        <div className="lg:w-1/3 w-full bg-primary text-white rounded-xl shadow-xl p-8 space-y-5">
          <LogoMark className="w-2/3 m-auto" />
